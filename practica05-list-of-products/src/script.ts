...
const divListProduct: HTMLDivElement = document.getElementById("list-products") as HTMLDivElement;
createProducts();
...
function createProducts(): void{
    ...
    listProducts.map((product: Product) => {
        ...
        let imgDIV: HTMLDivElement = createProductImg(product.image);
        let nameDIV: HTMLDivElement = createProductName(product.title, product.category);
        let priceDIV: HTMLDivElement = createProductPrice(product.price, product.stock);
        ...
        return divProduct;
    }).forEach((divProduct)=>divListProduct.appendChild(divProduct));
}
function createProductImg(image: string): HTMLDivElement{
...
function createProductName(title: string, category: string): HTMLDivElement{
...
function createProductPrice(price: number, stock: number): HTMLDivElement{

    let spanStock: HTMLSpanElement = document.createElement("span");
    let [label1, classes1]: [label: string, classes: string] = checkStock(stock);
...
function checkStock(stock: number): [label: string, classes: string]{

    if(stock == 0) return ['outofstock','badge text-bg-danger']
    else if( stock < 10) return ['lowstock','badge text-bg-warning']
